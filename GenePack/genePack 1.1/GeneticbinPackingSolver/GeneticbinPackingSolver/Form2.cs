...
namespace GeneticbinPackingSolver
{
    public partial class Form2 : Form
    {
        ...
        public static string [] evaluationcheck(string [] genepool , int h , int w )
        {
            for(int i = 0; i < genepool.Length ;i++)
            {
                for (int j = 0; j < genepool[i].Length; j++)
                {
                    ...
                    if (genepool[i][j] == '0')
                    {
                        if (bhClone[j] > h || bwClone[j] > w )
                        {

                            genepool = genepool.Where((source, index) => index != i).ToArray();
                            break;
                        }
                    }
                    else if (genepool[i][j] == '1')

                    {
                        if (bhClone[j] > w || bwClone[j] > h)
                        {
                                genepool = genepool.Where((source, index) => index != i ).ToArray();
                            break;
                        }
        ...
        */
            ...
            // dims of container 
            Console.WriteLine("length = " + h + "     width = " + w);

            // first generation 
            string[] genePool = poolMaker(n, populationSize, r);
            genePool = evaluationcheck(genePool,h,w);
      ...
      public static string findnames (int x , int [] oh , int [] ow)
        ...
        public string calcU(int ub)
        {
            double u = 0; 
            for (int i = 0; i < n; i ++)
            {
                u = u + Convert.ToDouble(bwidth[i]) * Convert.ToDouble(bhight[i]);
            }
           
            return  Convert.ToString(   100*( Math.Round( u / (Convert.ToDouble(ub) * (Convert.ToDouble(hight) * Convert.ToDouble(width) )  ) , 4) )) + " %";
        }
